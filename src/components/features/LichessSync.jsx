...
import { Link2, Clock, TrendingUp, Puzzle, Timer, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
const LichessSync = ({ currentUser, userData }) => {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            ...
            try {
                ...
                if (json.success) {
                    // Merge rapid and puzzle data by date
                    ...
                    // Take last 30 data points for readability
    ...
    // Calculate growth from ratingHistory
    ...
    // ─── RENDER: No username or rejected ───
    if (!status || status === LICHESS_STATUS.REJECTED) {
        return (
            <div className="lichess-sync-card content-card">
                <div className="lichess-header">
                    <div className="lichess-icon-wrap">
                        <Link2 size={20} />
                    </div>
                    <div>
                        <h3 className="lichess-title">Lichess Sync</h3>
                        <p className="lichess-subtitle">Link your Lichess account to track ratings</p>
                    </div>
                </div>
                {status === LICHESS_STATUS.REJECTED && (
                    <div className="lichess-rejected-notice">
                        <AlertCircle size={16} />
                        <span>Your previous request was rejected. You can try again.</span>
                    </div>
                )}
    ...
    if (status === LICHESS_STATUS.PENDING) {
        return (
            <div className="lichess-sync-card content-card">
                <div className="lichess-header">
                    <div className="lichess-icon-wrap pending">
                        <Clock size={20} />
                    </div>
                    <div>
                        <h3 className="lichess-title">Lichess Sync</h3>
                        <p className="lichess-subtitle">Linked as <strong>{linkedUsername}</strong></p>
                    </div>
                </div>
                <div className="lichess-pending-body">
                    ...
                    <p className="pending-desc">
                        Your coach will review and approve your Lichess account. Once approved, your ratings will sync automatically.
                    </p>
                </div>
            </div>
        );
    ...
    // ─── RENDER: Approved — Analytics View ───
    return (
        <div className="lichess-sync-card content-card lichess-approved">
            <div className="lichess-header">
                <div className="lichess-icon-wrap approved">
                ...
                <div className="lichess-synced-badge">
                    <Timer size={12} />
                    <span>Synced: {lastSyncedStr}</span>
                </div>
            </div>
            ...
            <div className="lichess-puzzle-target">
                ...
                <div className="puzzle-target-bar-bg">
                    <div
                        className="puzzle-target-bar-fill"
                        style={{ width: `${Math.min(((userData?.weeklyPuzzlesSolved || 0) / 200) * 100, 100)}%` }}
                    />
                </div>
                <div className="puzzle-target-label">
                    {userData?.weeklyPuzzlesSolved || 0} / 200 puzzles this week
                </div>
            </div>
        </div>
    );
